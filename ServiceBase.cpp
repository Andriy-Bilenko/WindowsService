...
PWSTR ServiceBase::m_name = const_cast<PWSTR>(L"");

// static service name getter
PWSTR ServiceBase::GetName()
{
    return m_name;
}
...
void ServiceBase::Shutdown()
{
    ...
    {
        // Log the error.
        WriteErrorLogEntry(const_cast<PWSTR>(L"Service Shutdown"), dwError);
    }
    catch (...)
    {
        // Log the error.
        WriteEventLogEntry(const_cast<PWSTR>(L"Service failed to shut down."), EVENTLOG_ERROR_TYPE);
    }
}
